const GalleryCard = ({ imageSrc, title, onOpenModal, className }) => {
  return (
    <div 
          ...
          <img src={imageSrc} className="w-full h-full object-cover transition-all duration-700 group-hover:scale-105"onError={(e) => { e.target.parentElement.style.display = 'none'; }} />

      <div 
        onClick={onOpenModal}
        className="absolute inset-0 z-10 bg-black/70 flex flex-col items-center justify-center 
                   opacity-0 group-hover:opacity-100 transition-all duration-500 ease-in-out backdrop-blur-[2px]">
        <div className="px-5 py-2 border z-10 border-main bg-black/80 backdrop-blur-md -skew-x-12 transform transition-transform duration-500 group-hover:scale-100 scale-90">
