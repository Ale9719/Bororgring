...
const Cards = ({ title, subtitle, buttonText, children, link }) => {
  return (
    <div
      style={{ filter: 'drop-shadow(0 0 10px var(--shadow-main))' }}
      className="group relative transition-all duration-300">
      <div
        className="bg-main p-0.5"
        ...
        <div
          ...
          {link && buttonText && (
            <Link
              to={link}
              className="block w-full text-center border border-main py-2 text-sm font-bold uppercase lg:hover:bg-main lg:hover:text-black transition-colors">
              {buttonText}
            </Link>
          )}
        </div>
